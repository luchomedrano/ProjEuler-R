...
#ggl style sheet for R dice usar cammel case para fun y explicitar return. https://github.com/google/styleguide/blob/gh-pages/Rguide.md
#Vertical alignment
#indentation

EsMultiplo<- function(x,y){
...
ExtraePares  <- function(x){
  
  return(x[which((x/2)%%1==0)])
  
}

print(ExtraePares(30:34))
...
ElapsedTime <- system.time({
  ...
  #menor a 1000 asi que defino 999 como tope
  #armo las secuencias de múltiplos de 3 y 5, las concateno 
  #y extraigo los valores comunes a las 2 series con "unique"
  #Sumo el vector de valores únicos.
  
  tope      <- 999
  
  seq3      <- seq(from = 0, to = tope, by = 3)
  
  seq5      <- seq(from = 0, to = tope, by = 5)
...
# Fibo suma n y n-1 para dar n+1, 1, 1, 2, 3, 5, 8, 13... muy rápido crece y pasa los 4M, en el n=34
# F0=0 y F1=1
...
ElapsedTime <- system.time({
  ...
  Fibo<- numeric(33)          #vector numérico
  Fibo[1] <- Fibo[2] <- 1     #inicio serie
  
  for (i in 3:33) { 
    Fibo[i] <- Fibo[i - 2] + Fibo[i - 1]
  }
  
  # aca querría solucionarlo solo con el dato de que f0=0 y f1=1 y no asumir que ya calculé que el 33 es el tope. 
  ...
  x[length(x)-1] # el while corta cuando se deja de cumplir la condición, osea necesito el n anteúltimo.
  x2<- x[1:length(x)-1]
  y1<-which((Fibo/2)%%1==0) 
  
  respuesta<-sum(Fibo[y1])
  print(respuesta)
